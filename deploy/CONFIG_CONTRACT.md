...
### Public/Member Config Overrides (Option A)

These fields allow a **single Events page** to render differently based on login state.
The widget detects whether the user is logged in and merges the appropriate config.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `publicConfig` | object | `{}` | Settings applied when user is NOT logged in. |
| `memberConfig` | object | `{}` | Settings applied when user IS logged in. |

**How It Works:**

1. Widget calls WA API to check if user is logged in
2. If logged in: `memberConfig` is merged into base config
3. If not logged in: `publicConfig` is merged into base config

**Example:**

```javascript
window.CLUBCALENDAR_CONFIG = {
    // Base settings (apply to everyone)
    headerTitle: 'Club Events',
    showFilters: true,

    // Applied when NOT logged in
    publicConfig: {
        showMyEvents: false,           // Can't show "My Events" if not logged in
        quickFilters: { public: false } // All visible events are public anyway
    },
...
Any config field can be overridden. Common overrides:

- `showMyEvents` - Hide for public, show for members
- `quickFilters.*` - Different filter buttons per audience
- `dropdownFilters.*` - Different dropdowns per audience
- `headerTitle` - Different titles per audience
...
CLUBCALENDAR_CONFIG = {
    ...
    // PUBLIC/MEMBER OVERRIDES (Option A - single page)
    publicConfig: { /* any config fields */ },
    memberConfig: { /* any config fields */ },
...
| 2024-12-26 | Added publicConfig/memberConfig for Option A (single page) |
